...
`go-starter` - Go-библиотека для запуска воркеров сервиса с graceful shutdown.
...
`go-starter` берет эту координацию на себя.
...
`go-starter` разделяет ответственность:
...
```bash
go get github.com/overiss/go-starter
```
...
import (
	...
	starter "github.com/overiss/go-starter"
)
...
Внутри `go-starter` сделан упор на предсказуемую работу под нагрузкой:
